...
for readp in read_pairs:
    ...
    if len(read_pairs[readp]) >= 1:
        ...
        if len(read_pairs[readp]) > 1:
            tmp_str +="%s --genomeDir %s --readFilesIn %s %s --readFilesCommand zcat --limitSjdbInsertNsj 2000000 --limitOutSJcollapsed 2000000 --runThreadN %s --genomeLoad LoadAndKeep --outSAMtype None --outFileNamePrefix %s\n"%(star_engine,ref,read1,read2,cpu_,"%s_%s"%(readp,name))
        else:
            tmp_str +="%s --genomeDir %s --readFilesIn %s --readFilesCommand zcat --runThreadN %s --limitSjdbInsertNsj 2000000 --limitOutSJcollapsed 2000000 --genomeLoad LoadAndKeep --outSAMtype None --outFileNamePrefix %s\n"%(star_engine,ref,read1,cpu_,"%s_%s"%(readp,name))
