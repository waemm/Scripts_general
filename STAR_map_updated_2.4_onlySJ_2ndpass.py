...
for readp in read_pairs:
    ...
    if len(read_pairs[readp]) >= 1:
        ...
        if len(read_pairs[readp]) > 1:
            tmp_str +="%s --genomeDir %s --readFilesIn %s %s --sjdbFileChrStartEnd %s --limitSjdbInsertNsj 2000000 --limitOutSJcollapsed 2000000  --readFilesCommand zcat --runThreadN %s --genomeLoad LoadAndKeep --outSAMtype None --outFileNamePrefix %s\n"%(star_engine,ref,read1,read2,sjdb,cpu_,"%s_%s"%(readp,name))
        else:
            tmp_str +="%s --genomeDir %s --readFilesIn %s --sjdbFileChrStartEnd %s --readFilesCommand zcat --runThreadN %s --genomeLoad LoadAndKeep --limitSjdbInsertNsj 2000000 --limitOutSJcollapsed 2000000 --outSAMtype None --outFileNamePrefix %s\n"%(star_engine,ref,read1,sjdb,cpu_,"%s_%s"%(readp,name))
