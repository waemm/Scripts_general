...
raw_read_dir = os.getcwd() # sys.argv[1]

out_dir = os.getcwd() #sys.argv[2]

cpu_ = sys.argv[1]

name = sys.argv[2]

read_ending = sys.argv[3]

ref = sys.argv[4]

id_sep = sys.argv[5]

id_index = map(int,sys.argv[6].split(","))
...
if ref == "hg":
    ...
    ref = "/cluster/scratch3/vyp-scratch2/reference_datasets/RNASeq/Mouse/STAR"
...
for readp in read_pairs:
    ...
    if len(read_pairs[readp]) >= 1:
        ...
        if len(read_pairs[readp]) > 1:
            tmp_str +="%s --genomeDir %s --twopassMode Basic  --readFilesIn %s %s --readFilesCommand zcat  --outSAMtype BAM SortedByCoordinate --limitSjdbInsertNsj 2000000 --limitOutSJcollapsed 2000000 --runThreadN %s --genomeLoad NoSharedMemory --outFileNamePrefix %s\n"%(star_engine,ref,read1,read2,cpu_,"%s_%s"%(readp,name))
        else:
            tmp_str +="%s --genomeDir %s --twopassMode Basic  --readFilesIn %s --readFilesCommand zcat  --outSAMtype BAM SortedByCoordinate --runThreadN %s --limitSjdbInsertNsj 2000000 --limitOutSJcollapsed 2000000 --genomeLoad NoSharedMemory --outFileNamePrefix %s\n"%(star_engine,ref,read1,cpu_,"%s_%s"%(readp,name))
