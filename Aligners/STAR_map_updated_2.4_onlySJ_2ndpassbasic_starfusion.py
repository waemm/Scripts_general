...
for readp in read_pairs:
    ...
    if len(read_pairs[readp]) >= 1:
        ...
        if len(read_pairs[readp]) > 1:
            tmp_str +="%s --genomeDir %s --twopassMode Basic --outReadsUnmapped None --chimSegmentMin 12 \
            --chimJunctionOverhangMin 12 --alignSJDBoverhangMin 10 --alignMatesGapMax 1000000\
            --alignIntronMax 200000 --chimSegmentReadGapMax parameter 3 \
            --alignSJstitchMismatchNmax 5 -1 5 5 \
            --limitBAMsortRAM 31532137230 \
            --outSAMtype BAM SortedByCoordinate \
            --readFilesIn %s %s --readFilesCommand zcat --limitSjdbInsertNsj 2000000 --limitOutSJcollapsed 2000000 --runThreadN %s --genomeLoad NoSharedMemory --outFileNamePrefix %s\n"%(star_engine,ref,read1,read2,cpu_,"%s_%s"%(readp,name))
        else:
            tmp_str +="%s --genomeDir %s --twopassMode Basic --outReadsUnmapped None --chimSegmentMin 12 \
            --chimJunctionOverhangMin 12 --alignSJDBoverhangMin 10 --alignMatesGapMax 1000000\
            --alignIntronMax 200000 --chimSegmentReadGapMax parameter 3 \
            ...
            --limitBAMsortRAM 31532137230 \
            --outSAMtype BAM SortedByCoordinate \
            --readFilesIn %s --readFilesCommand zcat --runThreadN %s --limitSjdbInsertNsj 2000000 --limitOutSJcollapsed 2000000 --genomeLoad NoSharedMemory --outFileNamePrefix %s\n"%(star_engine,ref,read1,cpu_,"%s_%s"%(readp,name))
